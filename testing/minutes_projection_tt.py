...
def find_minutes_projection(season_game_logs, curr_scoreboard, positions_df, season_start_date, curr_date, player_id, conn):
    ...
    def find_minute_projection_features(conn, season_start_date, curr_date, season_game_logs, curr_scoreboard, positions_df, player_id):
        ...
        try:
            ...
            print(f"Couldn't find a scoreboard row for {player_id}")
            return []
    ...
    if minutes_projection_features.empty:
    ...
    print(f"Minutes projection for {player_id} is {minutes_projection}")
...
if __name__ == "__main__":
    ...
    curr_date = datetime.strptime(season_start_date, "%Y-%m-%d").date()
    end_date = date.today()

    while curr_date < end_date:

        curr_box_scores = season_game_logs[
            (season_game_logs['GAME_DATE'] == str(curr_date))
        ]
        ...
        if curr_box_scores.empty:
            
            print(f"Could not find box scores for {curr_date}")
            curr_date += timedelta(days=1)
            continue
            
        curr_game_ids = curr_box_scores.drop_duplicates("GAME_ID")['GAME_ID'].to_list()

        for game_id in curr_game_ids:

            game_box_score = curr_box_scores[curr_box_scores['GAME_ID'] == game_id]

            player_ids = game_box_score['PLAYER_ID'].to_list()

            for player_id in player_ids:

                print(f"Updating props training table for {player_id} on {curr_date}")
                curr_minutes_projection = find_minutes_projection(
                season_game_logs=season_game_logs, 
                curr_scoreboard=curr_scoreboard, 
                positions_df=positions_df, 
                season_start_date=season_start_date, 
                ...
                if curr_minutes_projection == -1:

                    continue

                cursor.execute("""
                               UPDATE PROPS_TRAINING_TABLE 
                               SET MINUTES_PROJECTION = ?
                                    WHERE GAME_ID = ? 
                                    AND PLAYER_ID = ?
                            """, (float(curr_minutes_projection), game_id, player_id)
                )

        curr_date += timedelta(days=1)
    
    conn.commit()
    print(f"Finished updating minutes projection for training table.")
