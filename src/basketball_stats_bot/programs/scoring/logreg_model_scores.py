    ...
    for prop in props:
        ...
        features_dict = {
            ...
            prop: {
                ...
                'LAST_GAME': last_5_games_overall[0],
                'SECOND_LAST_GAME': last_5_games_overall[1],
                'THIRD_LAST_GAME': last_5_games_overall[2],
                'FOURTH_LAST_GAME': last_5_games_overall[3],
                'FIFTH_LAST_GAME': last_5_games_overall[4],
                'AVERAGE_LAST_20': average_overall_last_20,
                'LAST_GAME_VS_OPP': last_5_games_vs_opp[0],
                'SECOND_LAST_GAME_VS_OPP': last_5_games_vs_opp[1],
                'THIRD_LAST_GAME_VS_OPP': last_5_games_vs_opp[2],
                'FOURTH_LAST_GAME_VS_OPP': last_5_games_vs_opp[3],
                'FIFTH_LAST_GAME_VS_OPP': last_5_games_vs_opp[4],
...
if __name__ == "__main__":
    ...
    for player_id, values in today_prop_features.items():
        ...
        for hashmap in values['features']:
            ...
            for prop, feature in hashmap.items():
                ...
                models_path = r"C:\Users\noahs\.vscode\basketball stats bot\main\training\model\logistic_regression"

                scaler_file_path = os.path.join(models_path, f"{prop}_logreg_scaler.pkl")
                model_file_path = os.path.join(models_path, f"{prop}_logreg_model.pkl")
                scaler = joblib.load(scaler_file_path)
                model = joblib.load(model_file_path)

                today_scaled = scaler.transform(df)

                prob_over = model.predict_proba(today_scaled)[0][1]
    ...
    with open(f"scoringv4_{curr_date}.json", "w") as f:
