...
def train_xgboost_with_usages(conn):

    config = load_config()

    def train_model(conn, prop):

        df = pd.read_sql_query("SELECT * FROM TRAINING_TABLE_WITH_NAN WHERE PROP = ?", conn, params=(prop,))
        ...
        feature_cols.append(f"AVERAGE_LAST_5_EXPECTED_{prop}_MINUS_LINE")
        feature_cols.append(f"AVERAGE_LAST_10_EXPECTED_{prop}_MINUS_LINE")
        ...
        curr_path = os.path.join(config.XGBOOST_PATH, f"{prop}_xgboost_model_with_usages.pkl")
    ...
    for prop in props:
        ...
        print(f"Training XGBoost Model with usage values for {prop}..")
...
if __name__ == "__main__":
    ...
    train_xgboost_with_usages(conn=conn)
